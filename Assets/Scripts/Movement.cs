...
public class Movement : MonoBehaviour{
    ...
    void Update() {
        if (objectTransform.position.y >= maximum){
            ...
            Vector3 directionToMove = new Vector3(targetPosition.x - transform.position.x, targetPosition.y - objectTransform.position.y, 0f);
            transform.Translate(directionToMove.normalized * rychlost * Time.deltaTime);
       }
    }
}